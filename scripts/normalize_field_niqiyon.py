...
def is_niqiyon_related(field):
    if not field or not isinstance(field, str):
        return False
    f = field.strip()
    return any(kw in f for kw in NIQIYON_KEYWORDS)
...
def main():
    ...
    for e in entries:
        if not isinstance(e, dict):
            continue
        field = e.get("field", "")
        if is_niqiyon_related(field) and field != TARGET_FIELD:
            e["field"] = TARGET_FIELD
            updated += 1
    with open(ENTRIES_PATH, "w", encoding="utf-8") as f:
        json.dump(entries, f, ensure_ascii=False, indent=2)
    print("Updated", updated, "entries to field 'Niqiyon'")
